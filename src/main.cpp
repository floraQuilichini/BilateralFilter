...
int main(int argc, char **argv)
{
	...
	{
		std::string ext = FileIO::getFileExt(infile.c_str()); // get file extension
		std::cout << "ext : " << ext << std::endl;
		if (ext.compare(".pcd") == 0)
			ok = FileIO::readAndSortOrientedPointsFromPCDFile(infile.c_str(), outfile.c_str(), octree, radius);
		else
