...
bool FileIO::readAndSortUnorientedPointsFromPCDFile(const char* input_filename, const char* output_filename, Octree& octree, double min_radius)
{
	ifstream in;
	...
	if (!in)
	{
		std::cerr << "File " << input_filename << " could not be opened" << std::endl;
		return false;
	}
	...
	do {
		...
		int pos = in.tellg();
		diff += 1;
		//std::cout << "file line : " << header_line << std::endl;
		//std::cout << "pos : " << pos << std::endl;
	} while (header_line.compare("DATA ascii") != 0);

	// write header 
	writePCDHeaderFile(output_filename, line_vec);
	...
	in.clear();
	in.seekg((pos-diff)+1, ios::beg);//starting back from the beginning of ascii data 
/*	getline(in, firstline);
	std::cout << "first line : " << firstline << std::endl;*/
...
bool FileIO::readAndSortOrientedPointsFromPCDFile(const char* input_filename, const char* output_filename, Octree& octree, double min_radius)
{
	ifstream in;
	in.open(input_filename);
	...
	if (nword < 6)
	{
		cerr << "File must contain at least 6 doubles per line (x y z nx ny nz)" << endl;
		return false;
	}
...
{
	ofstream out;
	std::string ext = getFileExt(filename); // get file extension
	if (ext.compare(".pcd") == 0)
	{
		out.open(filename, std::ofstream::out | std::ofstream::app);
		out.precision(numeric_limits<float>::max_digits10);
	}
	else
	{
		out.open(filename);
...
void FileIO::writePCDHeaderFile(const char* output_filename, std::vector<string>& line_vec)
{
	...
	// write header
	pcd_file.open(output_filename, std::ios::out);
	for (size_t i = 0; i < line_vec.size(); i++) {
		pcd_file << line_vec[i] << std::endl;
	}
