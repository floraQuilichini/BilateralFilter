...
class FileIO
{
   ...
   /** @brief get file extension from a full path string
   */
   static std::string getFileExt(const std::string& s)
   {
	   ...
	   if (i != std::string::npos) {
		   ...
		   return(name_ext.substr(lastindex));
	   }
   ...
   /** @brief read points from a pcd file with unoriented points
   * @param filename name of the file to read points from
   * @param octree to sort and store the points in
   * @param min_radius if positive, create the octree such that the smallest
   * cell has size 2*min_radius
   ...
   */
   ...
   static bool readAndSortUnorientedPointsFromPCDFile(const char* filename, const char* output_filename, Octree& octree,
								double min_radius = -1);
   ...
   /** @brief read points from a pcd file with points with normals
   * @param filename name of the file to read points from
   * @param octree to sort and store the points in
   * @param min_radius if positive, create the octree such that the smallest
   * cell has size 2*min_radius
   * @return true if saving was successful
   */
   static bool FileIO::readAndSortOrientedPointsFromPCDFile(const char* input_filename, const char* output_filename, Octree& octree,
								double min_radius = -1);
   ...
   /* @brief write content of an ascii file in a pcd file format
   */
   static void writePCDHeaderFile(const char* pcd_filename, std::vector<std::string>& line_vec);
